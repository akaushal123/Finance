...
def quote():
    ...
    if request.method == "POST":
        ...
        #returning the quoted template
        return render_template("quoted.html", name =quoted['name'], sym = quoted['symbol'], price = usd(quoted['price']))
...
def register():
    ...
    if request.method == "GET":
        ...
        #stores info of user in database
        user_register = db.execute("INSERT INTO users (username, hash) VALUES (:username, :password)", username = request.form.get("username"), password = pwd_context.encrypt(request.form.get("password")))
